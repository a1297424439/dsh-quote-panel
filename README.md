...
- **🐂 牛来桌宠（v1.1 新增）**：看板上点 "🐂 牛来" 即可召唤一头会撒欢奔跑的牛，盯 A股 / 美股自选；任一标的 `changePct > 0` 就跑两步 + 文字喊两声 `\\牛来/`。提供三种版本：
  - **网页版**：直接浮在 DSH 会话页面，跟着页面滚动，自选自动同步
  - **弹窗版**：通过 `window.open` 弹独立无边框小窗（共享同源 `/dshq/quotes`）
  - **Electron 桌面版（v1.2 新增）**：真透明置顶桌面宠物，可拖动、点击互动、右键菜单，详见下文
...
看板头部新增一个 **🐂 牛来** 按钮，打开后一头牛会浮在页面底部左右徘徊。每当自选列表里**任一标的实时上涨（`changePct > 0`）**，牛立刻：
...
| 模式 | 入口 | 适合场景 |
|---|---|---|
| 网页版（默认） | 看板顶部点 **🐂 牛来** | 在 DSH 会话页随时看，最轻量 |
| 桌面版（独立窗口） | 牛来对泡右下角点 **🪟 弹出独立窗口** | 当桌面宠物长期挂着；浏览器窗口可置顶 |

桌面版底层逻辑与网页版一致，唯一差别是通过 `window.open` 弹一个无边框 `popup` 窗口（默认 480×360，可任意缩放）。它依然走 `/dshq/quotes`（同源），所以实时性不打折；如果你直接双击 `lib/desk-pet/standalone.html` 打开（脱离 DSH 同源），会自动进入 **Demo 模式**：牛会随机假报上涨，纯视觉演示，并在状态栏提示「请从 DSH 会话页面启动此窗口以获得实时行情」。

### 关闭桌宠

- 看板标题里那个高亮按钮变回 **🐂 关掉牛** 即可关闭；或对泡左上 **✕**

## 🐮 真·桌面桌宠（Electron 版，v1.2 新增 · v1.3 修仙瞬移 · v1.4 满屏飞仙）

除了上面的网页版/弹窗版，仓库还附带一个 **Electron 独立桌面应用**（`desktop-pet/` 目录）——真透明窗口、无边框、置顶，牛直接站在你的桌面上：

- **满屏飞行（v1.4）**：窗口为铺满整个工作区的透明层，牛在**全屏幕范围**内瞬移飞行（高度可达屏幕 82%），不再局限于小窗内"原地扑腾"
- **鼠标穿透不挡操作**：全屏透明层默认把鼠标事件穿透给下层应用（`setIgnoreMouseEvents` + forward），只有鼠标悬停到牛身上才接管交互——正常办公打游戏零干扰，窗口也不占任务栏
- **透明无边框置顶窗口**：角色素材已自动抠掉白底（`desktop-pet/assets/cow-view-*.png`，5 视角全套）
- **左键拖动**：按住牛即可拖到屏幕任意位置
- **修仙瞬移（v1.3）**：平时不再走路，而是**带残影瞬移**——旧位置留下 4 道逐渐消散的牛影，落地时爆发一圈仙气粒子与气浪圈，高度也随机浮动
- **股票上涨喊牛来**：默认盯 A 股三大指数（腾讯财经公开接口），任一标的 `changePct > 0` 时**金光爆闪瞬移**到屏幕中部，跳起来喊 `\\牛来/`（最小间隔 8 秒 + 70% 概率，不会刷屏）
- **点击互动**：单击牛轮流触发 **跳跃 → 压扁回弹 → 左右抖动**，每次配一条随机趣味中文语录气泡（不透明底、自动收敛在屏幕内不裁字；牛飞到屏幕上半截时气泡自动挪到脚下）
- **右键菜单**：选择关注股票（预设 + 自定义）、调整大小（小/中/大，控制牛本体尺寸）、置顶开关、退出程序

```bash
cd desktop-pet
npm install        # 国内可加 --registry=https://registry.npmmirror.com
npm start          # 启动桌宠
npm run dist       # 打包可执行文件（electron-builder）
```

设置（自选、大小、置顶）持久化在系统 userData 目录，重启不丢。
